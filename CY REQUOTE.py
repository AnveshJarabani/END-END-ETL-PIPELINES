...
OLD_QTS=pd.read_hdf('CY_ADJ.H5',key='YT_3QTR')
SM=pd.read_hdf('ST_BM_BR.H5',key='SM_PARTS')
ind=np.array(QUOTES['TOP LEVEL'].unique()).tolist()
...
QUOTES.loc[QUOTES['COST EA']=='NEW','COST EA']=QUOTES['ACT']
QUOTES.loc[QUOTES['COST EXT']=='NEW','COST EXT']=QUOTES['ACT EXT']
...
# QUOTES=QUOTES.pivot_table(index=['TOP LEVEL', 'P/N','DESC','MAKE/BUY','TYPE','COST EA','ACT'],values=['QTY EXT',
#                 'COST EXT' ,'ACT EXT'],aggfunc=np.sum)
# QUOTES.reset_index(inplace=True)
QUOTES.drop(columns=['LAST VENDOR'],inplace=True)
...
QUOTES.loc[QUOTES['P/N']=='CY-207754','NEW COST EA']=2475
QUOTES.loc[QUOTES['P/N']=='CY-207754','NEW COST EXT']=2475*QUOTES['QTY EXT']
QUOTES['COMMENTS']=''
CLMS=QUOTES.columns
PREV_QUOTES=pd.read_pickle('CY-HPARTS.PKL')
# PREV_QUOTES=PREV_QUOTES.pivot_table(index=['TOP LEVEL', 'P/N','DESC','MAKE/BUY','TYPE','OLD COST EA','NEW COST EA'],values=['QTY EXT',
#             'OLD COST EXT','NEW COST EXT'],aggfunc=np.sum)
# PREV_QUOTES.reset_index(inplace=True)
PREV_QUOTES=PREV_QUOTES[['TOP LEVEL','P/N','QTY EXT','OLD COST EA','NEW COST EA','OLD COST EXT','NEW COST EXT']]
PREV_QUOTES.columns=['TOP LEVEL','P/N','QTY EXT','OLD COST EA_OLD','NEW COST EA_OLD','OLD COST EXT_OLD','NEW COST EXT_OLD',]
QUOTES=QUOTES.merge(PREV_QUOTES,left_on=['TOP LEVEL','P/N','QTY EXT'],right_on=['TOP LEVEL','P/N','QTY EXT'],how='left')
QUOTES=QUOTES.drop_duplicates(ignore_index=True)
QUOTES['OLD COST EA']=QUOTES['OLD COST EA_OLD']
QUOTES['OLD COST EXT']=QUOTES['OLD COST EXT_OLD']
QUOTES['NEW COST EA']=QUOTES['NEW COST EA_OLD']
QUOTES['NEW COST EXT']=QUOTES['NEW COST EXT_OLD']
...
QUOTES=QUOTES[CLMS]
# THIS IS A TEMPORARY CHANGE TO CONTAIN THESE BELOW VENDORS AS H PARTS
QUOTES.loc[QUOTES['LAST VENDOR'].str.contains('NEWPORT|CYMER|MEANS|MATHESON|DELTA GROUP|BENCHMARK'),'TYPE']='H-PART'
QUOTES.loc[QUOTES['LAST VENDOR'].str.contains('NEWPORT|CYMER|MEANS|MATHESON|DELTA GROUP|BENCHMARK'),'MAKE/BUY']='H-PART'
